...
async function serveSnapshot(
	...
	if (clientGeneration === undefined || currentGeneration !== clientGeneration || waitMs <= 0) {
		...
		logger.debug("auth-broker snapshot served", {
			peer,
	...
	if (currentGeneration !== clientGeneration) {
		const body = buildSnapshot(storage, refresher);
		logger.debug("auth-broker snapshot long-poll changed", {
			peer,
			credentials: body.credentials.length,
			generation: body.generation,
		});
	...
	logger.debug("auth-broker snapshot long-poll unchanged", { peer, generation: currentGeneration });
		...
		fetch: async (req): Promise<Response> => {
			...
			try {
				...
				if (!isAuthorized(req, tokens)) {
					logger.debug("auth-broker request unauthorized", { method: req.method, path: pathname, peer });
				...
				if (req.method === "GET" && pathname === "/v1/usage") {
					try {
						...
						logger.debug("auth-broker usage served", { peer, reports: trimmed.length });
				...
				if (refreshMatch) {
					...
					try {
						...
						logger.debug("auth-broker credential refreshed", {
							id,
				...
				if (disableMatch) {
					...
					if (!ok) {
						logger.debug("auth-broker disable miss", { id, peer, cause });
						return json(404, { error: `No credential with id=${id}` });
					}
					logger.debug("auth-broker credential disabled", { id, peer, cause });
					...
					try {
						...
						const identity =
							credential.type === "oauth"
								...
								: "(api key)";
						logger.debug("auth-broker credential upserted", {
							provider,
