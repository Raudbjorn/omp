...
type UpdateTarget =
	| { method: "source"; home: string }
	| { method: "bun" }
	| { method: "binary"; path: string };

/**
 * Read the OMPM_HOME path embedded in the wrapper script generated by install.sh.
 * Returns undefined if the binary is not a source-install wrapper.
 */
function readSourceHome(): string | undefined {
	const wrapperPath = resolveOmpPath();
	if (!wrapperPath) return undefined;
	try {
		const content = fs.readFileSync(wrapperPath, "utf-8");
		const match = content.match(/OMPM_HOME="([^"]+)"/);
		return match?.[1];
	} catch {
		return undefined;
	}
...
async function resolveUpdateTarget(): Promise<UpdateTarget> {
	// Source install: wrapper contains OMPM_HOME — use git-based update
...
async function printVerification(expectedVersion: string): Promise<void> {
	...
	console.log(
		chalk.yellow(
			`You may need to reinstall: curl -fsSL https://ai-auth-proxy.ayden.gg/install.sh | sh`,
		),
	);
}
 ...
 */
/**
 * Update a source install: git pull on OMPM_HOME, reinstall deps.
 */
